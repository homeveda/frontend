...
const UpdateInspectionPage = () => {
  ...
  const [categories, setCategories] = useState({
    plumbing: { status: "Pending", video: null, preview: null, existing: null },
    electricity: { status: "Pending", video: null, preview: null, existing: null },
    chimneyPoint: { status: "Pending", video: null, preview: null, existing: null },
    falseCeiling: { status: "Pending", video: null, preview: null, existing: null },
    flooring: { status: "Pending", video: null, preview: null, existing: null },
  });
  ...
  const [removedOtherVideoIndices, setRemovedOtherVideoIndices] = useState([]);
    ...
    const fetchInspection = async () => {
      try {
        const response = await axios.get(
          `${backendUrl}/inspections/${projectId}`,
          {
            headers: {
        ...
        const inspectionData = response.data.inspections[0];
        if (inspectionData) {
          setInspection(inspectionData);
          setInspectionDate(
            inspectionData.inspectionDate
              ? inspectionData.inspectionDate.split("T")[0]
              : new Date().toISOString().split("T")[0]
          );
          setReadyForNextPhase(inspectionData.readyForNextPhase || false);

          // Set category data
          setCategories({
            plumbing: {
              status: inspectionData.plumbingStatus || "Pending",
              video: null,
              preview: null,
              existing: inspectionData.plumbingVideo,
            },
            electricity: {
              status: inspectionData.electricityStatus || "Pending",
              video: null,
              preview: null,
              existing: inspectionData.electricityVideo,
            },
            chimneyPoint: {
              status: inspectionData.chimneyPointStatus || "Pending",
              video: null,
              preview: null,
              existing: inspectionData.chimneyPointVideo,
            },
            falseCeiling: {
              status: inspectionData.falseCeilingStatus || "Pending",
              video: null,
              preview: null,
              existing: inspectionData.falseCeilingVideo,
            },
            flooring: {
              status: inspectionData.flooringStatus || "Pending",
              video: null,
              preview: null,
              existing: inspectionData.flooringVideo,
            },
          });

          setOtherVideos(inspectionData.otherVideos || []);
        }
      } catch (error) {
        console.error("Error fetching inspection:", error);
        triggerPopup("Failed to fetch inspection", "red");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleRemoveCategoryVideo = (category) => {
    setCategories((prev) => ({
      ...prev,
      [category]: {
        ...prev[category],
  ...
  const handleRemoveOtherVideo = (index) => {
    setRemovedOtherVideoIndices((prev) => [...prev, index]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `${backendUrl}/inspections/${inspection._id}`,
        formData,
        {
          headers: {
            Authorization: adminToken ? `Bearer ${adminToken}` : undefined,
      ...
      triggerPopup("Inspection updated successfully!", "green");
      setTimeout(() => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          <p className="text-lg text-gray-600 mb-4">No inspection found</p>
          <button
            onClick={() => router.back()}
            className="bg-[#e07b63] text-white px-6 py-2 rounded hover:bg-[#d56a52]"
          >
            Go Back
          </button>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-0 sm:px-2">
                ...
                {Object.entries(categories).map(([key, value]) => (
                  ...
                  {value.existing && !value.video && (
                    <div className="mb-3">
                      ...
                      <video
                        src={value.existing}
                        className="w-full h-20 sm:h-24 object-cover rounded border border-gray-300 mb-2"
                        controls
                      />
                      <button
                        type="button"
                        onClick={() => handleRemoveCategoryVideo(key)}
                  ...
                  {!value.existing || value.video ? (
                    <div>
                      <label className="block text-xs text-gray-600 mb-2">
                        {value.video || value.preview ? "Replace Video" : "Upload Video"}
                      </label>
                      <input
                        type="file"
                        accept="video/*"
                    ...
                    <button
                      ...
                      onClick={() => {
                        setCategories((prev) => ({
                          ...prev,
                          [key]: { ...prev[key], video: new File([], "temp") },
                        }));
                      }}
                      className="w-full bg-[#e07b63] text-white px-2 py-1 rounded text-xs hover:bg-[#d56a52] mt-2 transition-colors"
                    >
                      Replace Video
                    </button>
                  )}
                  ...
                  {otherVideos.map((video, index) => (
                    ...
                    >
                      <p className="text-xs text-gray-600 mb-1 truncate">
                        {video instanceof File ? video.name : `Video ${index + 1}`}
                      </p>
                      {video instanceof File ? (
                        <video
                          src={URL.createObjectURL(video)}
                          className="w-full h-12 sm:h-16 object-cover rounded mb-1"
                        />
                      ) : (
                        <video
                          src={video}
          ...
          <div className="mb-6 sm:mb-8 flex items-center gap-3 bg-gray-50 p-3 sm:p-4 rounded-lg border border-gray-200">
            <input
              ...
              className="w-4 h-4 sm:w-5 sm:h-5 cursor-pointer flex-shrink-0 accent-[#e07b63]"
            />
            <label
